...
def lr_cost_function(feature, theta, Y, classifier, lmbda):
    ...
    for i in range(0, no_of_rows):
        ...
        J = J + -(y[i] * math.log(hypothesis_val) + (1 - y[i]) * math.log(1 - hypothesis_val)) + (
                    lmbda / 2) * regularized_cost
...
def gradient_descent(feature, Y, theta, alpha, lmbda, classifier, iters):
    ...
    for k in range(0, iters):
        ...
        if (k < 10):
            print 'iteration = ' + str(k) + ' cost=' + str(
                lr_cost_function(feature, theta, Y, classifier, lmbda)) + '  class=' + str(classifier)
        else:
